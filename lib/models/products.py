...
class Product(BaseModel):
    ...
    def create(cls, name, quantity, category_id, supplier_id):
        ...
        cls.execute_query(
            "INSERT INTO product (name, quantity, category_id, supplier_id) VALUES (?, ?, ?, ?)",
            (name, quantity, category_id, supplier_id)
        )
        return cls(name, quantity, category_id, supplier_id)
    @classmethod
    def delete(cls, product_id):
        cls.execute_query("DELETE FROM product WHERE id=?", (product_id,))
    @classmethod
    def get_all(cls):
